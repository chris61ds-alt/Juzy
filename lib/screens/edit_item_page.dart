...
class _EditItemPageState extends State<EditItemPage> with TickerProviderStateMixin {
              ...
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: Theme.of(context).colorScheme.surfaceVariant, shape: BoxShape.circle, border: Border.all(color: _juzyColor.withOpacity(0.5))),
  ...
  Widget _buildStep1(ColorScheme colors, Color fillColor, Color borderColor) {
    return SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(children: [
      GestureDetector(onTap: _showMediaMenu, child: Stack(alignment: Alignment.bottomRight, children: [Container(height: 120, width: 120, alignment: Alignment.center, decoration: BoxDecoration(color: colors.surface, borderRadius: BorderRadius.circular(30), border: Border.all(color: _juzyColor.withOpacity(0.5), width: 2), image: _imagePath != null ? DecorationImage(image: FileImage(File(_imagePath!)), fit: BoxFit.cover) : null), child: _imagePath == null ? Text(_emoji ?? "📦", style: const TextStyle(fontSize: 50)) : null), Container(margin: const EdgeInsets.all(5), padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: colors.surface, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 4)]), child: Icon(Icons.edit, size: 14, color: _juzyColor))])), 
      const SizedBox(height: 20), 
      
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(T.get('label_category'), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)), 
        IconButton(
          onPressed: () => setState(() => _isDeletingCategory = !_isDeletingCategory), 
          icon: Icon(_isDeletingCategory ? Icons.check_circle : Icons.edit, color: _isDeletingCategory ? _juzyColor : Colors.grey, size: 20),
          tooltip: T.get('manage_categories'),
        )
      ]), 
      const SizedBox(height: 10), 
      ...
      Wrap(spacing: 8, runSpacing: 8, children: [
        ...widget.availableCategories.map((cat) { 
          ...
          return GestureDetector(
            onTap: () {
              if(_isDeletingCategory) {
                if(!isStandard) _deleteCategory(cat); 
              } else {
                _applySmartDefaults(cat); 
              }
            },
            child: Chip(
              ...
              labelStyle: TextStyle(
                color: _isDeletingCategory 
                  ? (isStandard ? Colors.grey : Colors.white)
                  : (isSelected ? Colors.white : colors.onSurface),
  ...
  Widget _buildStep3(ColorScheme colors) {
    ...
    String displayTime = "";
    if (_lifespanDays < 30) {
      displayTime = "${_lifespanDays.toInt()} ${T.get('days')}";
    } else if (_lifespanDays < 365) {
      displayTime = "${(_lifespanDays/30).toStringAsFixed(1)} ${T.get('months')}";
    } else {
      displayTime = "${(_lifespanDays/365).toStringAsFixed(1)} ${T.get('years')}";
    }

    return Padding(padding: const EdgeInsets.all(24), child: Column(children: [
      Text(T.get('wish_lifespan'), style: const TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
      Padding(padding: const EdgeInsets.only(top: 5, bottom: 20), child: Text(T.get('lifespan_hint'), style: TextStyle(fontSize: 13, color: Colors.grey.withOpacity(0.8), fontStyle: FontStyle.italic))),
      ...
      Slider(
        ...
        min: 1, 
        max: 3650, 
        activeColor: _juzyColor, 
        onChanged: (v) => setState(() => _lifespanDays = v)
      ),
      
      const SizedBox(height: 10),
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
        decoration: BoxDecoration(color: _juzyColor.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
        child: Text("${T.get('calc_date')} $dateString", style: TextStyle(color: _juzyColor, fontWeight: FontWeight.bold))
      ),
